...
export function DepartmentsTable({ refreshKey }: { refreshKey: number }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(json.error ?? "Could not load departments.");
        setData(null);
        return;
      }
      setData({
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setActionError(json.error ?? "Could not delete department.");
        return;
      }
  ...
  async function onEditSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setActionError(json.error ?? "Could not update department.");
        return;
      }
      setEditTarget(null);
      await load();
  ...
  return (
    <section
      ...
      aria-labelledby={`${searchId}-departments-heading`}
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2
            id={`${searchId}-departments-heading`}
